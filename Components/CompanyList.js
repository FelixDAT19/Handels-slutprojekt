...
function CompanyList({company}) {// list with companies and what exact placements they have
  ...
  return (
    ...
    <label htmlFor="btn" className="first dropdownButton">Företag {/* a dropdown button that reveales all companies and thair locations */}
    <span className="fas fa-caret-down"></span>
    </label>
    <input type="checkbox" id="btn" className="dropdownCheckbox"/>
    <ul className="menuCompany firstUl">

    {company.map(({name,placement}, o ) /*  maps out all the companies */ => ( 
            <li key={o} className="listItems firstLI">
               ...
               <ul className="menuCompany secondUl">
                    {placement.map((i, key) /* puts the locations under its specific company */ => 
